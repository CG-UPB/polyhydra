...
Camera::Camera()
{
    ...
    world = glm::mat4(1.0f);
    view = glm::lookAt(position, target, m_world_up);
    m_sensitivity = 1.0f;
...
void Camera::update()
{
    ...
    projection = glm::perspective(glm::radians(zoom), m_screen_width / m_screen_height, near, far);
...
void Camera::handle_mouse_scroll(glm::vec2 scroll)
{
    ...
    else if (m_mode == CameraMode::ORBIT)
    {
        ...
        if (glm::length(target - new_pos) <= min_distance
            || (glm::dot((target - position), step) >= 0
                && glm::length(position - target) <= glm::length(position - new_pos)))
        {
