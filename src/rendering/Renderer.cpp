...
#include "polyhydra/util/StringUtil.h"
...
Renderer::Renderer(int width, int height)
{
    ...
    if (AppState::settings.multisampling)
    {
        buffers.target_framebuffer_ms
            = std::make_shared<FrameBufferObject>(width, height, FrameBufferObject::RGBA_AND_DEPTH_MULTISAMPLE);
    }
    else
    {
        buffers.target_framebuffer_ms
            = std::make_shared<FrameBufferObject>(width, height, FrameBufferObject::RGBA_AND_DEPTH);
    }
    ...
    buffers.selection_frame_buffer
        = std::make_shared<FrameBufferObject>(width / 2, height / 2, FrameBufferObject::RGBA_AND_DEPTH);
...
void Renderer::render(const RenderData& data)
{
    ...
    FrameBufferObject::copy(
        GL_NONE, GL_NONE, GL_DEPTH_BUFFER_BIT, buffers.target_framebuffer_ms, buffers.target_framebuffer);
    ...
    FrameBufferObject::copy(
        GL_COLOR_ATTACHMENT0, GL_COLOR_BUFFER_BIT, buffers.target_framebuffer_ms, buffers.target_framebuffer);
    FrameBufferObject::copy(
        GL_NONE, GL_NONE, GL_DEPTH_BUFFER_BIT, buffers.target_framebuffer_ms, buffers.target_framebuffer);
    ...
    if (ImGui::IsWindowHovered() && !input_blocking)
    {
        if (auto mesh = mesh_list->get_focused_mesh())
        {
            ...
            else if (Input::key_down(Input::ROTATE_MESH))
            {
                ...
                {
                    auto axis = TrackBall::get_rotation_axis(
                        {input.last.x, input.last.y}, {input.pos.x, input.pos.y}, camera->get_viewport_size());
                    auto angle = TrackBall::get_rotation_angle(
                        {input.last.x, input.last.y}, {input.pos.x, input.pos.y}, camera->get_viewport_size());
                    // glm::mat3 camera_to_trackball = glm::inverse(mesh->get_data().get_transform()) *
                    // glm::mat3(cam.world));
...
void Renderer::export_image(const std::string& path, const ExportOptions& options)
{
    ...
    auto export_framebuffer_ms = std::make_shared<FrameBufferObject>(
        export_width, export_height, FrameBufferObject::RGBA_AND_DEPTH_MULTISAMPLE);
    auto export_framebuffer
        = std::make_shared<FrameBufferObject>(export_width, export_height, FrameBufferObject::RGBA_AND_DEPTH);
    auto export_post_framebuffer
        = std::make_shared<FrameBufferObject>(export_width, export_height, FrameBufferObject::RGBA_AND_DEPTH);
