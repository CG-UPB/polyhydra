...
SelectionHoverPass::SelectionHoverPass() : m_hover_color(glm::vec4(0.9, 0.2, 0.2, 0.5))
{
    ...
    // Create VAOs
    m_quad_vao
        = std::make_unique<VertexArrayObject>(CommonMeshes::PlaneXY::vertices(), CommonMeshes::PlaneXY::indices());
    ...
    for (const auto& mesh : renderer.render_list)
    {
        ...
        if (m_hovered_type == SELECTION_TYPE_VERTEX)
        {
            ...
            if (m_quad_vao != nullptr)
            {
                ...
                m_quad_circle_shader->set_uniform_float("u_average_cell_size",
                                                        mesh->get_mvb()->get_average_cell_size());
                m_quad_vao->draw();
