...
void ShadowMapPass::generate_cascade_textures()
{
    ...
    std::vector<FrameBufferAttachment> attachments = {FrameBufferAttachment{.internal_format = GL_RGBA,
                                                                            .format = GL_RGBA,
...
void ShadowMapPass::render(const Renderer& renderer)
{
    ...
    if (cam->near != m_current_near || cam->far != m_current_far
        || settings.num_shadow_cascades != m_current_cascade_level)
    {
    ...
    for (int i = 0; i < m_current_cascade_level; i++)
    {
        ...
        for (const auto& mesh : renderer.render_list)
        {
            ...
            if (is_bezier_mesh)
            {
                ...
                m_shadow_shader->set_uniform_int(
                    "u_bezier_degree",
                ...
                m_shadow_shader->set_uniform_texbuffer("u_control_points_tb", mtb->get_binding(), mtb->get_texture());
...
void ShadowMapPass::calculate_cascade(const Renderer& renderer, float near, float far, int i)
{
    ...
    const auto proj = glm::perspective((float)glm::radians(cam->zoom), (float)m_width / (float)m_height, near, far);
    ...
    for (uint32_t x = 0; x < 2; ++x)
    {
        for (uint32_t y = 0; y < 2; ++y)
        {
            ...
            {
                glm::vec4 corner
                    = inverse * glm::vec4(2.0f * (float)x - 1.0f, 2.0f * (float)y - 1.0f, 2.0f * (float)z - 1.0f, 1.0f);
