...
const SSAOOptions SSAOPass::QUALITY_SSAO
    = {.active = true, .num_samples = 64, .sample_radius = 1.5, .strength = 1.5, .z_bias = 0.01};

// balance between image quality and performance
const SSAOOptions SSAOPass::BALANCED_SSAO
    = {.active = true, .num_samples = 32, .sample_radius = 1.5, .strength = 1.5, .z_bias = 0.015};

// lowest image quality, but least performance impact
const SSAOOptions SSAOPass::PERFORMANCE_SSAO
    = {.active = true, .num_samples = 16, .sample_radius = 1.5, .strength = 1.5, .z_bias = 0.02};

SSAOPass::SSAOPass(int initial_width, int initial_height) : m_options(SSAOPass::QUALITY_SSAO)
{
    // we only need one channel for the occlusion factor
    std::vector<FrameBufferAttachment> ssao_attachments = {FrameBufferAttachment{.internal_format = GL_RED,
                                                                                 .format = GL_RED,
                                                                                 .type = GL_FLOAT,
                                                                                 .attachment = GL_COLOR_ATTACHMENT0,
                                                                                 .texture_filter = GL_NEAREST}};
    // for the blur factor, we only need one channel as well
    std::vector<FrameBufferAttachment> blur_attachments = {FrameBufferAttachment{.internal_format = GL_RED,
                                                                                 .format = GL_RED,
                                                                                 .type = GL_FLOAT,
                                                                                 .attachment = GL_COLOR_ATTACHMENT0,
                                                                                 .texture_filter = GL_NEAREST}};
...
void SSAOPass::generate_sample_kernel()
{
    ...
    while (m_sample_kernel.size() < SSAOPass::s_max_samples)
    {
        ...
        glm::vec3 sample(get_random_float(-1.0, 1.0), get_random_float(-1.0, 1.0), get_random_float(0.0, 1.0));
