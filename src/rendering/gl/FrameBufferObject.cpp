...
const std::vector<FrameBufferAttachment> FrameBufferObject::RGBA_AND_DEPTH = {
    // color
    FrameBufferAttachment{.internal_format = GL_RGBA,
                          .format = GL_RGBA,
    ...
    // depth
    FrameBufferAttachment{.internal_format = GL_DEPTH_COMPONENT,
...
const std::vector<FrameBufferAttachment> FrameBufferObject::RGBA_AND_DEPTH_MULTISAMPLE = {
    ...
    FrameBufferAttachment{.internal_format = GL_RGBA,
                          ...
                          .multisample = true},
    // depth
    FrameBufferAttachment{.internal_format = GL_DEPTH24_STENCIL8,
...
void FrameBufferObject::copy(int attachment,
                             int mask,
...
void FrameBufferObject::copy(int read_attachment,
                             int draw_attachment,
                             int mask,
                             const std::shared_ptr<FrameBufferObject>& src,
                             const std::shared_ptr<FrameBufferObject>& dest)
...
uint32_t FrameBufferObject::create_attachment(const FrameBufferAttachment& attachment) const
{
    ...
    if (attachment.multisample)
    {
        ...
        glTexImage2DMultisample(
            GL_TEXTURE_2D_MULTISAMPLE, s_num_samples, attachment.internal_format, m_width, m_height, GL_TRUE);
