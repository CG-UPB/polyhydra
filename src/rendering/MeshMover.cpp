...
namespace vOS{
    ...
    void MeshMover::update()
    {
        ...
        if(mouse_down && !m_mouse_press_flank && move_mode_desired) {
            ...
            if(Window::instance().get_all_mesh_ids()->size() == 1)
            ...
            if(mesh_id >= 0) {
                ...
                if (m_target_mesh_object == nullptr) {
                    ...
                    m_mesh_original_position = m_target_mesh_object->get_data().m_position;
                }
            }else{
    ...
    void MeshMover::move_mesh()
    {
        ...
        if(m_target_mesh_object != nullptr)
        {
            ...
            glm::vec3 cam_space = screen_difference.x * m_camera->get_right() + screen_difference.y * m_camera->get_up() + screen_difference.z * m_camera->get_front();

            // Convert Camera Space to new World Position
            glm::vec3 new_position = m_mesh_original_position + cam_space;

            // Set World Position
            m_target_mesh_object->get_data().m_position = new_position;
        }
    }
}