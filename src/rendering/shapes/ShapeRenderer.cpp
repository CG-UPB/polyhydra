...
void ShapeRenderer::render(const Renderer& renderer)
{
    ...
    for (auto& [mesh_id, types] : m_render_data_by_mesh_by_type)
    {
        ...
        if (mesh_id != INVALID_MESH_ID)
        {
            ...
            m_shape_shader->set_uniform_vec3f("u_min", data.bb_min);
            m_shape_shader->set_uniform_vec3f("u_max", data.bb_max);
            m_shape_shader->set_uniform_vec3f("u_slice_direction", data.slice_direction);
...
void ShapeRenderer::add_shape(std::unique_ptr<BaseShape>&& shape)
{
    ...
    m_on_before_render_commands.emplace_back(
        [this, shape_id]
        {
