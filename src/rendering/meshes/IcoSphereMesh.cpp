...
#include "polyhydra/rendering/meshes/IcoSphereMesh.h"
...
IcoSphereMesh::IcoSphereMesh(int recursion_level) : m_index(0)
{
    ...
    for (int i = 0; i < recursion_level; i++)
    {
        ...
        for (const Face& face : faces)
        {
            ...
            uint32_t a = get_mid_point(face.v0, face.v1);
            uint32_t b = get_mid_point(face.v1, face.v2);
            uint32_t c = get_mid_point(face.v2, face.v0);
...
uint32_t IcoSphereMesh::get_mid_point(uint32_t v0, uint32_t v1)
{
    ...
    // add vertex to cache
    uint32_t mid_index = add_vertex(mid_x, mid_y, mid_z);
    m_mid_point_cache.emplace(key, mid_index);
    return mid_index;
}

uint32_t IcoSphereMesh::add_vertex(float x, float y, float z)
{
...
const std::vector<uint32_t>& IcoSphereMesh::indices()
{
    return m_indices;
}
