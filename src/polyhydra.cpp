#define POLYHYDRA_IGNORE_INVALID_MESH
#ifndef POLYHYDRA_IGNORE_INVALID_MESH
#define POLYHYDRA_ASSERT_VALID_MESH(mesh, list) assert(list->get_mesh(mesh.get_id()) && "Invalid VMesh handle")
#else
#define POLYHYDRA_ASSERT_VALID_MESH(mesh, list)                                                                        \
    if (!list->get_mesh(mesh.get_id()))                                                                                \
    return
#endif
...
#include "polyhydra/mesh/MeshSerializer.h"
...
void init()
{
    window = std::make_unique<Internal::Window>(1280, 720, "polyhydra");
    window->initialize();
...
VMesh load_from_dialog(const std::string& title, const char* name)
{
    ...
    if (auto file = polyhydra::file_dialog(title))
    {
        return polyhydra::load(file, name);
    }
    return VMesh(-1);
}
...
{
    commands.emplace_back([mesh]() { mesh_list->set_focused_mesh(mesh.get_id()); });
}
...
{
    commands.emplace_back([mesh, name] { mesh_list->set_name(mesh.get_id(), name); });
}
...
{
    commands.emplace_back([mesh, mode] { mesh_list->set_rendering_mode(mesh.get_id(), mode); });
}

void render_cells(const VMesh& mesh, bool cells)
{
    commands.emplace_back([mesh, cells] { mesh_list->render_cells(mesh.get_id(), cells); });
}

void render_lines(const VMesh& mesh, bool lines)
{
    commands.emplace_back([mesh, lines] { mesh_list->render_lines(mesh.get_id(), lines); });
}

void render_points(const VMesh& mesh, bool points)
{
    commands.emplace_back([mesh, points] { mesh_list->render_points(mesh.get_id(), points); });
}
...
{
    commands.emplace_back([mesh, size] { mesh_list->set_point_size(mesh.get_id(), size); });
}
...
{
    commands.emplace_back([mesh, width] { mesh_list->set_line_width(mesh.get_id(), width); });
}
...
{
    commands.emplace_back([mesh, mode] { mesh_list->set_shading_mode(mesh.get_id(), mode); });
}
...
{
    commands.emplace_back([mode] { Internal::AppState::settings.selection_mode = mode; });
}
...
{
    commands.emplace_back([x, y, z] { camera->look_at(glm::vec3(x, y, z)); });
}
...
{
    commands.emplace_back([intensity] { Internal::AppState::settings.light.intensity = intensity; });
}
...
{
    commands.emplace_back([gamma] { Internal::AppState::settings.post_processing.gamma = gamma; });
}
...
{
    commands.emplace_back([saturation] { Internal::AppState::settings.post_processing.saturation = saturation; });
}
...
{
    commands.emplace_back([contrast] { Internal::AppState::settings.post_processing.contrast = contrast; });
}
...
{
    commands.emplace_back([ground] { Internal::AppState::settings.ground.solid = ground; });
}
...
{
    commands.emplace_back([grid] { Internal::AppState::settings.ground.grid = grid; });
}
...
{
    commands.emplace_back([height] { Internal::AppState::settings.ground.height = height; });
}
...
{
    commands.emplace_back([shadows] { Internal::AppState::settings.shadows_active = shadows; });
}
...
void set_shadow_cascades(int cascades)
{
    ...
    commands.emplace_back([cascades] { Internal::AppState::settings.num_shadow_cascades = cascades; });
}
...
{
    commands.emplace_back([strength] { Internal::AppState::settings.shadow.shadow_strength = strength; });
}
...
{
    commands.emplace_back([penumbra] { Internal::AppState::settings.shadow.penumbra_scale = penumbra; });
}
...
{
    commands.emplace_back([ssao] { Internal::AppState::settings.ssao_active = ssao; });
}
...
{
    commands.emplace_back([mode] { Internal::AppState::settings.ssao_mode = mode; });
}
...
{
    commands.emplace_back([transparency] { Internal::AppState::settings.transparency_active = transparency; });
}
...
{
    commands.emplace_back([mode] { Internal::AppState::settings.transparency_mode = mode; });
}
...
void set_transparency_passes(int passes)
{
    ...
    commands.emplace_back([passes] { Internal::AppState::settings.num_depth_peeling_passes = passes; });
}
...
void update(const VMesh& mesh, const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, KernelType>* instance)
{
    ...
    *ovm = *instance;
    commands.emplace_back([mesh, ovm] { mesh_list->set_mesh(mesh.get_id(), ovm); });
}

void update(const VMesh& mesh, const std::string& path)
{
    commands.emplace_back([mesh, path] { mesh_list->set_mesh(mesh.get_id(), path); });
}
...
void serialize(const VMesh& mesh, const std::string& filename)
{
    commands.emplace_back(
        [mesh, filename]
        {
            mesh_list->execute_for_mesh([mesh, filename](const std::shared_ptr<Internal::MeshObject>& m)
...
{
    commands.emplace_back([mesh] { mesh_list->delete_mesh(mesh.get_id()); });
}

void clear()
{
    commands.emplace_back([] { mesh_list->delete_meshes(); });
}

std::vector<VMesh> get_meshes()
{
    std::vector<VMesh> meshes;
    mesh_list->iterate([&meshes](auto id, auto mesh) { meshes.push_back(VMesh(id)); });
    return meshes;
}
...
{
    commands.emplace_back([mesh, vertex] { mesh_list->select(EntityType::Vertex, mesh.get_id(), vertex.idx()); });
}

void select(const VMesh& mesh, OpenVolumeMesh::EdgeHandle edge)
{
    commands.emplace_back([mesh, edge] { mesh_list->select(EntityType::Edge, mesh.get_id(), edge.idx()); });
}

void select(const VMesh& mesh, OpenVolumeMesh::HalfFaceHandle halfface)
{
    commands.emplace_back([mesh, halfface] { mesh_list->select(EntityType::Halfface, mesh.get_id(), halfface.idx()); });
}

void select(const VMesh& mesh, OpenVolumeMesh::FaceHandle face)
{
    commands.emplace_back([mesh, face] { mesh_list->select(EntityType::Face, mesh.get_id(), face.idx()); });
}

void select(const VMesh& mesh, OpenVolumeMesh::CellHandle cell)
{
    commands.emplace_back([mesh, cell] { mesh_list->select(EntityType::Cell, mesh.get_id(), cell.idx()); });
}

void deselect(const VMesh& mesh, OpenVolumeMesh::VertexHandle vertex)
{
    commands.emplace_back([mesh, vertex] { mesh_list->deselect(EntityType::Vertex, mesh.get_id(), vertex.idx()); });
}

void deselect(const VMesh& mesh, OpenVolumeMesh::EdgeHandle edge)
{
    commands.emplace_back([mesh, edge] { mesh_list->deselect(EntityType::Edge, mesh.get_id(), edge.idx()); });
}
...
{
    commands.emplace_back([mesh, face] { mesh_list->deselect(EntityType::Face, mesh.get_id(), face.idx()); });
}

void deselect(const VMesh& mesh, OpenVolumeMesh::CellHandle cell)
{
    commands.emplace_back([mesh, cell] { mesh_list->deselect(EntityType::Cell, mesh.get_id(), cell.idx()); });
}

void reset_selection(const VMesh& mesh)
{
    commands.emplace_back([mesh] { mesh_list->reset_selection(mesh.get_id()); });
}
...
{
    commands.emplace_back([mesh, ambient] { mesh_list->set_ambient(mesh.get_id(), ambient); });
}
...
{
    commands.emplace_back([mesh, diffuse] { mesh_list->set_diffuse(mesh.get_id(), diffuse); });
}
...
{
    commands.emplace_back([mesh, specular] { mesh_list->set_specular(mesh.get_id(), specular); });
}
...
{
    commands.emplace_back([mesh, coefficient] { mesh_list->set_specular_coefficient(mesh.get_id(), coefficient); });
}
...
{
    commands.emplace_back([mesh, metallic] { mesh_list->get_mesh(mesh.get_id())->get_data().metallic = metallic; });
}
...
{
    commands.emplace_back([mesh, roughness] { mesh_list->get_mesh(mesh.get_id())->get_data().roughness = roughness; });
}
...
{
    commands.emplace_back([mesh, x, y, z] { mesh_list->set_position(mesh.get_id(), x, y, z); });
}
...
{
    commands.emplace_back([mesh, scale] { mesh_list->set_scale(mesh.get_id(), scale); });
}
...
{
    commands.emplace_back([mesh, x, y, z] { mesh_list->set_rotation(mesh.get_id(), x, y, z); });
}
...
{
    commands.emplace_back([mesh] { mesh_list->reset_rotation(mesh.get_id()); });
}

void set_slice_factor(const VMesh& mesh, const float level)
{
    commands.emplace_back([mesh, level] { mesh_list->set_slice_factor(mesh.get_id(), level); });
}
...
{
    commands.emplace_back([mesh, locked] { mesh_list->set_slice_lock(mesh.get_id(), locked); });
}
...
{
    commands.emplace_back([mesh, level] { mesh_list->set_peel_level(mesh.get_id(), level); });
}
...
{
    commands.emplace_back([mesh, reverse] { mesh_list->set_reverse_peeling(mesh.get_id(), reverse); });
}
...
{
    commands.emplace_back([mesh, rounding] { mesh_list->set_cell_rounding(mesh.get_id(), rounding); });
}
...
{
    commands.emplace_back([mesh, level] { mesh_list->set_tessellation_level(mesh.get_id(), level); });
}
...
{
    commands.emplace_back([mesh, size] { mesh_list->set_cell_size(mesh.get_id(), size); });
}
...
{
    commands.emplace_back([mesh, cell, visible]() { mesh_list->set_visibility(mesh.get_id(), cell, visible); });
}
...
{
    commands.emplace_back([mesh, visible] { mesh_list->set_visibility(mesh.get_id(), visible); });
}
...
{
    commands.emplace_back([mesh, cell] { mesh_list->isolate(mesh.get_id(), cell); });
}

void dig(const VMesh& mesh, OpenVolumeMesh::CellHandle cell)
{
    commands.emplace_back([mesh, cell] { mesh_list->hide(mesh.get_id(), cell); });
}

void never_discard(const VMesh& mesh, OpenVolumeMesh::CellHandle cell)
{
    commands.emplace_back([mesh, cell] { mesh_list->never_discard(mesh.get_id(), cell); });
}
...
void log(const std::string& message)
{
    commands.emplace_back(
        [message]
        {
            ...
            polyhydra::Internal::Log::info(message);
        });
}
...
void warn(const std::string& message)
{
    commands.emplace_back(
        [message]
        {
            ...
            polyhydra::Internal::Log::warn(message);
        });
}
...
void error(const std::string& message)
{
    commands.emplace_back(
        [message]
        {
            ...
            polyhydra::Internal::Log::error(message);
        });
}

void clear_logs()
{
    commands.emplace_back([] { window->panels.mesh_view->log_window->clear_logs(); });
}

void use_log_window(bool use)
{
    commands.emplace_back([use] { window->panels.mesh_view->log_window->hide_log_window(!use); });
}
...
{
    int id = shape.get_id();
    commands.emplace_back([id] { shapes->remove_shape(id); });
}

void remove_shapes()
{
    commands.emplace_back([] { shapes->remove_all(); });
}

void set_position(const VShape& shape, float x, float y, float z)
{
    commands.emplace_back([shape, x, y, z] { shapes->set_position(shape.get_id(), x, y, z); });
}
...
{
    commands.emplace_back([shape, scalar] { shapes->set_scale(shape.get_id(), scalar, scalar, scalar); });
}

void set_scale(const VShape& shape, float x, float y, float z)
{
    commands.emplace_back([shape, x, y, z] { shapes->set_scale(shape.get_id(), x, y, z); });
}
...
{
    commands.emplace_back([mode] { Internal::AppState::settings.shapes.use_pbr = static_cast<bool>(mode); });
}
...
{
    commands.emplace_back([ambient] { Internal::AppState::settings.shapes.ambient_strength = ambient; });
}
...
{
    commands.emplace_back([diffuse] { Internal::AppState::settings.shapes.diffuse_strength = diffuse; });
}
...
{
    commands.emplace_back([specular] { Internal::AppState::settings.shapes.specular_strength = specular; });
}
...
{
    commands.emplace_back([coefficient] { Internal::AppState::settings.shapes.specular_exponent = coefficient; });
}
...
{
    commands.emplace_back([metallic] { Internal::AppState::settings.shapes.metallic = metallic; });
}
...
{
    commands.emplace_back([roughness] { Internal::AppState::settings.shapes.roughness = roughness; });
}
...
template VMesh load<OpenVolumeMesh::TopologyKernel>(
    const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::TopologyKernel>*, const char*);
template VMesh load<OpenVolumeMesh::TetrahedralMeshTopologyKernel>(
    const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::TetrahedralMeshTopologyKernel>*,
    const char*);
template VMesh load<OpenVolumeMesh::HexahedralMeshTopologyKernel>(
    const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::HexahedralMeshTopologyKernel>*,
    const char*);
...
template void update<OpenVolumeMesh::TopologyKernel>(
    const VMesh&, const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::TopologyKernel>*);
template void update<OpenVolumeMesh::TetrahedralMeshTopologyKernel>(
    const VMesh&,
    const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::TetrahedralMeshTopologyKernel>*);
template void update<OpenVolumeMesh::HexahedralMeshTopologyKernel>(
    const VMesh&,
    const OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d, OpenVolumeMesh::HexahedralMeshTopologyKernel>*);
...
template void
set_color<OpenVolumeMesh::Vec4d>(const VMesh&, OpenVolumeMesh::HalfFaceHandle, const OpenVolumeMesh::Vec4d&);
template void
set_color<OpenVolumeMesh::Vec4f>(const VMesh&, OpenVolumeMesh::HalfFaceHandle, const OpenVolumeMesh::Vec4f&);
template void
set_color<std::array<double, 4>>(const VMesh&, OpenVolumeMesh::HalfFaceHandle, const std::array<double, 4>&);
template void
set_color<std::array<float, 4>>(const VMesh&, OpenVolumeMesh::HalfFaceHandle, const std::array<float, 4>&);
...
template OpenVolumeMesh::Vec3d get_transformed_point<OpenVolumeMesh::Vec3d>(const VMesh& mesh,
                                                                            const OpenVolumeMesh::Vec3d& point);
template OpenVolumeMesh::Vec3f get_transformed_point<OpenVolumeMesh::Vec3f>(const VMesh& mesh,
                                                                            const OpenVolumeMesh::Vec3f& point);
template std::array<double, 3> get_transformed_point<std::array<double, 3>>(const VMesh& mesh,
                                                                            const std::array<double, 3>& point);
template std::array<float, 3> get_transformed_point<std::array<float, 3>>(const VMesh& mesh,
