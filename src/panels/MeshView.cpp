...
#include "polyhydra/panels/MeshView.h"
...
{
MeshView::MeshView(int width, int height) : m_viewport_panel_width(width), m_viewport_panel_height(height)
{
    renderer = std::make_shared<Renderer>(width, height);
    renderer->set_selection_callback([&](int type, int id) { handle_mouse_hover(type, id); });
...
void MeshView::show()
{
    ...
    ImGui::GetWindowDrawList()->AddImage(reinterpret_cast<ImTextureID>(get_selected_texture()),
