#include "polyhydra/mesh/MeshVertexBuffer.h"
...
void MeshVertexBuffer::build_vertex_arrays()
{
    ...
    m_vao.vertex_only
        = std::make_shared<VertexArrayObject>(CommonMeshes::Sphere::vertices(), CommonMeshes::Sphere::indices());
...
void MeshVertexBuffer::add_vao_attributes(const std::shared_ptr<VertexArrayObject>& vao, VAO vao_id)
{
    ...
    for (auto& data : vao_locations)
    {
        ...
        {
            vao->add_attribute(attribute_data, attribute.location, attribute.element_count, attribute.per_instance);
        }
    }
...
void MeshVertexBuffer::load_next_cell()
{
    ...
    else
    {
        ...
        build_vertex_arrays();
        auto duration
            = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_loading_start)
                  .count();
...
void MeshVertexBuffer::add_cell_by_faces(const std::shared_ptr<OVMesh>& mesh, OVMCell cell)
{
    ...
    else
    {
        ...
        for (auto f : mesh->cell_faces(cell))
        {
            ...
            auto cp_prop
                = mesh->request_face_property<std::vector<double>>(MeshProperties::PROP_BEZIER_FACE_CONTROL_POINTS);
            ...
            cps.push_back(OpenVolumeMesh::Vec3d(cp_prop[f][3 * ((m + 2) * (m + 1) / 2) - 3],
                                                cp_prop[f][3 * ((m + 2) * (m + 1) / 2) - 2],
                                                cp_prop[f][3 * ((m + 2) * (m + 1) / 2) - 1]));
        ...
        {
            Log::warn("Wrong number of Bézier tetrahedron corner control points found: "
                      + std::to_string(corner_cps.size()) + ". Fallback to underlying OVM.");
    ...
    for (int i = 0; i < num_selection_vertices; i++)
    {
        ...
        VecUtil::push_vec2(get_attrib_array(VAO::SPHERE, Attribute::IS_DIGGED), glm::vec2(0.0f));
    ...
    for (auto ce_it : mesh->cell_edges(cell))
    {
        ...
        VecUtil::push_vec2(get_attrib_array(VAO::CYLINDER, Attribute::IS_DIGGED), glm::vec2(0.0f));
    ...
    for (auto chf_it : mesh->cell_halffaces(cell))
    {
        ...
        else if (halfface_vertices.size() > 3)
        {
            ...
            //     midpoint = 0.5f * glm::length(evec) * glm::vec3(halfface_vertices[(i+1)%halfface_vertices.size()] +
            //     halfface_vertices[i]); sum += glm::length(evec);
            // }
            // midpoint /= sum;
        ...
        add_halfface_index_and_count(
            VAO::MESH_FACE, halfface_id, m_vertex_offset_face, (int)halfface_data.vertices.size());
    ...
    for (const HalffaceData& halfface : halffaces)
    {
        ...
        for (const VertexData& vertex : halfface.vertices)
        {
            ...
            VecUtil::push_vec2(get_attrib_array(VAO::MESH_FACE, Attribute::IS_DIGGED), glm::vec2(0.0f));
            ;
            ...
            get_attrib_array(VAO::MESH_FACE, Attribute::IS_TRIANGLE)
                .push_back(is_bezier_mesh ? hf_ids[i] : is_triangle);
...
uint32_t MeshVertexBuffer::add_vertex_data_to_cell_data(RoundedCellData& data,
                                                        const float type,
                                                        const glm::vec3& pos,
                                                        ...
                                                        float min_edge_length)
{
    ...
    VecUtil::push_vec2(data.vertex_is_digged, glm::vec2(0.0f));
...
void MeshVertexBuffer::add_cell_rounded(const std::shared_ptr<OVMesh>& mesh, OVMCell cell)
{
    ...
    for (auto chf_it : mesh->cell_halffaces(cell))
    {
        ...
        for (auto hfhe_it : mesh->halfface_halfedges(chf_it))
        {
            ...
            cell_vertices[from_vertex.idx()].push_back(
                {from_vertex.idx(), mesh->to_vertex_handle(hfhe_it).idx(), hfhe_it.idx(), chf_it.idx()});
            ...
            do
            {
                ...
                for (auto chfe_it : mesh->halfface_edges(current_halfface))
                {
                    ...
                    if (found)
                    {
                        ...
                        cell_vertices[from_vertex.idx()].push_back({from_vertex.idx(),
    ...
    for (auto& it : cell_vertices)
    {
        ...
        for (size_t i = 0; i < vertex_data.size(); i++)
        {
            ...
            glm::vec3 edge_face_center_average
                = (face_centers[data.halfface_id] + face_centers[prev_data.halfface_id]) * 0.5f;
            ...
            // edge vertex
            halfedge_vertex_indices[edge.idx()][data.from_vertex_id]
                = add_vertex_data_to_cell_data(cell_data,
                                               VertexType::EDGE,
                                               corner_pos,
                                               -edge_normal,
                                               -edge_normal,
            ...
            halfface_vertices[data.halfface_id].push_back(VertexAttribData{
                ...
                RoundedFaceVertexData{
                    0, vertex_id, data.to_vertex_id, next_data.to_vertex_id, data.halfedge_id, next_data.halfedge_id}});
        }
        ...
        // corner vertex
        corner_vertex_indices[vertex_id] = add_vertex_data_to_cell_data(cell_data,
                                                                        VertexType::CORNER,
                                                                        corner_pos,
                                                                        -corner_normal,
                                                                        -corner_normal,
    ...
    for (auto& it : face_centers)
    {
        ...
        for (auto& attrib_data : halfface_vertices[halfface_id])
        {
            ...
            halfface_vertices_indices[halfface_id].push_back(
                RoundedFaceVertexData{face_vertex_index,
        ...
        glm::vec3 face_center_vertex_normal
            = use_vertex_normal ? glm::normalize(vertex_normal_average) : -face_normals[halfface_id];
        face_center_indices[halfface_id] = add_vertex_data_to_cell_data(cell_data,
                                                                        VertexType::CENTER,
                                                                        center_pos,
    ...
    for (const auto& it : halfface_vertices_indices)
    {
        ...
        for (const auto& face_vertex : face_vertices_data)
        {
            ...
            const int to_corner_vertex_edge_id
                = OVMesh::edge_handle(OpenVolumeMesh::HalfEdgeHandle{face_vertex.to_vertex_halfedge_id}).idx();
            const int next_to_corner_vertex_edge_id
                = OVMesh::edge_handle(OpenVolumeMesh::HalfEdgeHandle{face_vertex.next_to_vertex_halfedge_id}).idx();
            ...
            const uint32_t face_to_corner_vertex_index
                = corner_vertex_face_vertex_index[to_corner_vertex_id][halfface_id];

            const uint32_t to_corner_vertex_halfedge_index
                = halfedge_vertex_indices[to_corner_vertex_edge_id][corner_vertex_id];
            const uint32_t next_to_corner_vertex_halfedge_index
                = halfedge_vertex_indices[next_to_corner_vertex_edge_id][corner_vertex_id];
            const uint32_t to_vertex_halfedge_index
                = halfedge_vertex_indices[to_corner_vertex_edge_id][to_corner_vertex_id];

            // face center
            add_cell_triangle_indices(
                cell_data, face_center_index, face_corner_vertex_index, face_to_corner_vertex_index);

            // corner triangle
            add_cell_triangle_indices(
                cell_data, corner_vertex_index, face_corner_vertex_index, next_to_corner_vertex_halfedge_index);
            add_cell_triangle_indices(
                cell_data, corner_vertex_index, to_corner_vertex_halfedge_index, face_corner_vertex_index);

            // triangles along the edge
            add_cell_triangle_indices(
                cell_data, to_corner_vertex_halfedge_index, face_to_corner_vertex_index, face_corner_vertex_index);
            add_cell_triangle_indices(
                cell_data, to_corner_vertex_halfedge_index, to_vertex_halfedge_index, face_to_corner_vertex_index);
        }
    }
...
void MeshVertexBuffer::add_from_to_vertex(const std::shared_ptr<OVMesh>& mesh,
                                          const OpenVolumeMesh::VertexHandle& from,
                                          const OpenVolumeMesh::VertexHandle& to)
{
    VecUtil::push_vec3(get_attrib_array(VAO::CYLINDER, Attribute::SELECTION_FROM_VERTEX),
                       VecUtil::pos_to_vec3(*mesh, from));
    VecUtil::push_vec3(get_attrib_array(VAO::CYLINDER, Attribute::SELECTION_TO_VERTEX),
...
{
    return get_cell_attribute<glm::vec2>(VAO::MESH_FACE, Attribute::IS_DIGGED, cell_id)[0];
}

float MeshVertexBuffer::get_cell_never_discard(int cell_id)
{
    return get_cell_attribute<glm::vec2>(VAO::MESH_FACE, Attribute::IS_DIGGED, cell_id)[1];
}
...
void MeshVertexBuffer::set_cell_never_discard(int cell_id, bool never_discard)
{
    ...
    if (cell.is_valid())
    {
        glm::vec2 value = glm::vec2{get_cell_dig_value(cell_id), never_discard ? 1.0f : 0.0f};
...
void MeshVertexBuffer::set_cell_digged(int cell_id, bool digged)
{
    ...
    if (cell.is_valid())
    {
        glm::vec2 value = glm::vec2{digged ? 1.0f : 0.0f, get_cell_never_discard(cell_id)};
...
void MeshVertexBuffer::undo_digging(int count)
{
    ...
    {
        if (!undo_dig_stack.is_empty())
        {
            auto elem = undo_dig_stack.get_latest();
...
void MeshVertexBuffer::update_attribute(VAO vao_id, Attribute attribute, T value, int halfface_id, int cell_id)
{
    ...
    m_vao_update_data.push_back(AttributeUpdateData{vao_id, attribute, val, val_size, halfface_id, cell_id, false});
}
...
{
    auto [val, val_size] = get_value_and_size(value);
    m_vao_update_data.push_back({vao_id, attribute, val, val_size, -1, -1, true});
}

void MeshVertexBuffer::set_attribute_buffer(std::vector<float>& buffer,
                                            size_t offset,
                                            int value_size,
...
T MeshVertexBuffer::get_value_for_offset(VAO vao_id, Attribute attribute, int offset)
{
    ...
    {
        return glm::vec4{
            attrib_array[index], attrib_array[index + 1], attrib_array[index + 2], attrib_array[index + 3]};
    }
    return get_default_value<T>();
}
