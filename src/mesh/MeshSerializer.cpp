...
std::string MeshSerializer::serialize(MeshObject& mesh)
{
    ...
    nlohmann::json out;
    out["color"] = {{"r", data.color.r}, {"g", data.color.g}, {"b", data.color.b}, {"a", data.color.a}};
    out["selection_color"] = {{"r", data.selection_color.r},
    ...
    out["slice_direction"] = {{"x", mesh.m_slice_dir.x}, {"y", mesh.m_slice_dir.y}, {"z", mesh.m_slice_dir.z}};
    out["slice_bb"] = {{"min",
    ...
    out["position"] = {{"x", data.position.x}, {"y", data.position.y}, {"z", data.position.z}};
    out["scale"] = {{"x", data.scale.x}, {"y", data.scale.y}, {"z", data.scale.z}};
    out["rotation"] = {data.rotation[0][0],
                       data.rotation[0][1],
                       data.rotation[0][2],
                       data.rotation[0][3],
                       data.rotation[1][0],
                       ...
                       data.rotation[3][2],
                       data.rotation[3][3]};
    out["transformation"] = {data.transformation[0][0],
                             data.transformation[0][1],
                             data.transformation[0][2],
                             data.transformation[0][3],
                             data.transformation[1][0],
                             data.transformation[1][1],
                             data.transformation[1][2],
                             data.transformation[1][3],
                             data.transformation[2][0],
                             data.transformation[2][1],
...
void MeshSerializer::deserialize(MeshObject& mesh, const std::string& json_string)
{
    ...
    data.color = {in["color"]["r"], in["color"]["g"], in["color"]["b"], in["color"]["a"]};
    data.selection_color = {
        in["selection_color"]["r"], in["selection_color"]["g"], in["selection_color"]["b"], in["selection_color"]["a"]};
    ...
    mesh.m_transformed_bb
        = std::make_pair(glm::vec3{in["slice_bb"]["min"]["x"], in["slice_bb"]["min"]["y"], in["slice_bb"]["min"]["z"]},
    ...
    data.position = {in["position"]["x"], in["position"]["y"], in["position"]["z"]};
    data.scale = {in["scale"]["x"], in["scale"]["y"], in["scale"]["z"]};
    data.rotation = {{in["rotation"][0], in["rotation"][1], in["rotation"][2], in["rotation"][3]},
