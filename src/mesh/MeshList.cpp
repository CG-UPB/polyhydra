...
#include "polyhydra/util/Types.h"
...
void MeshList::set_mesh(const MeshID id,
                        const std::shared_ptr<OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d>>& ovm_mesh)
{
    auto f = [&ovm_mesh](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->set_mesh(ovm_mesh); };
...
{
    auto f = [this, id](const std::shared_ptr<MeshObject>& mesh) -> void { m_focused_mesh = id; };
    execute_for_mesh(f, id);
}
...
{
    auto f
        = [ambient](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().ambient_strength = ambient; };
    execute_for_mesh(f, id);
}
...
{
    auto f
        = [diffuse](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().diffuse_strength = diffuse; };
    execute_for_mesh(f, id);
}
...
{
    auto f = [this, &id](const std::shared_ptr<MeshObject>& mesh) -> void { get_mesh(id)->reset_rotation(); };
    execute_for_mesh(f, id);
}
...
void MeshList::set_slice_factor(const MeshID id, const float level)
{
    ...
    auto f = [level](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().slice_level = level; };
    execute_for_mesh(f, id);
}
...
{
    auto f = [reverse](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().reverse_peeling = reverse; };
    execute_for_mesh(f, id);
}
...
void MeshList::set_cell_rounding(MeshID id, float rounding)
{
    ...
    auto f = [rounding](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().rounding_size = rounding; };
    execute_for_mesh(f, id);
}
...
void MeshList::set_cell_size(const MeshID id, const float size)
{
    ...
    auto f = [size](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().cell_size = size; };
    execute_for_mesh(f, id);
}
...
void MeshList::set_tessellation_level(const MeshID id, const int level)
{
    ...
    auto f = [level](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().tessellation_level = level; };
    execute_for_mesh(f, id);
}
...
{
    auto f = [visible](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().visible = visible; };
    execute_for_mesh(f, id);
}
...
{
    auto f
        = [cell](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_mvb()->set_cell_isolated(cell.idx()); };
    execute_for_mesh(f, id);
}
...
{
    auto f = [name](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().name = name; };
    execute_for_mesh(f, id);
}
...
{
    auto f = [cells](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().cells = cells; };
    execute_for_mesh(f, id);
}

void MeshList::render_lines(MeshID id, bool lines)
{
    auto f = [lines](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().lines = lines; };
    execute_for_mesh(f, id);
}

void MeshList::render_points(MeshID id, bool points)
{
    auto f = [points](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().points = points; };
    execute_for_mesh(f, id);
}

void MeshList::set_point_size(MeshID id, float size)
{
    auto f = [size](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().point_size = size; };
    execute_for_mesh(f, id);
}

void MeshList::set_line_width(MeshID id, float width)
{
    auto f = [width](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().line_width = width; };
    execute_for_mesh(f, id);
}
...
{
    auto f = [mode](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_data().shading_mode = mode; };
    execute_for_mesh(f, id);
}

void polyhydra::Internal::MeshList::set_origin(polyhydra::Internal::MeshID id, const glm::vec3& origin)
{
    auto f = [origin](MeshID id, const std::shared_ptr<MeshObject>& mesh) -> void { mesh->set_origin(origin); };
    iterate(f);
}

void MeshList::set_color(const glm::vec4& color)
{
    auto f = [color](MeshID id, const std::shared_ptr<MeshObject>& mesh) -> void { mesh->set_mesh_color(color); };
    iterate(f);
}

void MeshList::set_color(const MeshID id, const glm::vec4& color)
{
    auto f = [color](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->set_mesh_color(color); };
    execute_for_mesh(f, id);
}
...
{
    auto f = [](const std::shared_ptr<MeshObject>& mesh) -> void { mesh->get_mvb()->reset_selection(); };
...
bool MeshList::is_bezier_mesh(polyhydra::Internal::MeshID id)
{
