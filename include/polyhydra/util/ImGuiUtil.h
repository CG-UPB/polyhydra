  ...
  public:
    static void icon(const std::string& icon, float size = ImGui::GetFontSize(), bool use_text_color = false)
    {
        ...
        if (use_text_color)
        {
            ...
            ImGui::Image(reinterpret_cast<ImTextureID>(icon_ref.get_id()),
                         {width, height},
                         {0.0f, 0.0f},
                         {1.0f, 1.0f},
                         text_color);
    ...
    static bool icon_button(const std::string& icon, float size = ImGui::GetFontSize(), bool use_text_color = true)
    {
        ...
        if (use_text_color)
        {
            return ImGui::ImageButton(reinterpret_cast<ImTextureID>(icon_ref.get_id()),
                                      {width, height},
                                      {0.0f, 0.0f},
                                      {1.0f, 1.0f},
                                      -1,
        ...
        return ImGui::ImageButton(reinterpret_cast<ImTextureID>(icon_ref.get_id()), {width, height});
    }
    ...
    static void add_background_rect(int num_items,
                                    uint32_t color = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Tab)))
    {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(
            ImVec2(p.x, p.y),
            ImVec2(c.x, ImGui::GetCursorScreenPos().y + ImGui::GetStyle().WindowPadding.y - 5 + border),
            border_color,
            ImGui::GetStyle().FrameRounding);
        ImGui::GetWindowDrawList()->AddRectFilled(
            ImVec2(p.x, p.y),
            ImVec2(c.x, ImGui::GetCursorScreenPos().y + ImGui::GetStyle().WindowPadding.y - 5),
            color,
            ImGui::GetStyle().FrameRounding);
        ImGui::SetCursorPos(
            {pos.x + ImGui::GetStyle().WindowPadding.x * 0.5f, pos.y + ImGui::GetStyle().WindowPadding.y * 0.5f});
    }

    static bool begin_menu_with_background(const std::string& name,
                                           int item_count,
