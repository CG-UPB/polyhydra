...
class MeshProperties
{
  public:
    ...
    static inline void setup_mesh_properties(OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d>& mesh)
    {
        ...
        add_property<std::vector<double>, OpenVolumeMesh::Entity::Face>(
            mesh, PROP_BEZIER_FACE_CONTROL_POINTS, std::vector<double>());
    }

  private:
    template <typename T, typename E>
    static inline void
    add_property(OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d>& mesh, const char* name, const T& default_value)
    {
