...
typedef std::map<OpenVolumeMesh::FaceHandle, std::vector<polyhydra::VSphere>> Face_CPs_Map;
typedef std::map<OpenVolumeMesh::FaceHandle, std::vector<polyhydra::VCylinder>> Face_Cylinder_Map;
...
void add_control_points_to_vector(std::vector<polyhydra::VSphere>& cps,
                                  polyhydra::VMesh& mesh,
                                  OpenVolumeMesh::FaceHandle face,
                                  float scale,
                                  glm::vec4 color = glm::vec4(0.9, 0.2, 0.2, 1.0))
{
    auto& ovm = *mesh.get_ovm();
    auto control_points_prop = ovm.request_face_property<std::vector<double>>("BezierFaceControlPoints");
    ...
    {
        auto control_point = mesh.add_shape<polyhydra::VSphere>();
...
void add_control_points_to_face(Face_CPs_Map& faces_cps,
                                polyhydra::VMesh& mesh,
                                OpenVolumeMesh::FaceHandle face,
                                float scale,
                                glm::vec4 color = glm::vec4(0.9, 0.2, 0.2, 1.0))
{
    std::vector<polyhydra::VSphere> cps;
    add_control_points_to_vector(cps, mesh, face, scale, color);
    faces_cps.emplace(face, cps);
}

void add_cp_connections_to_vector(std::vector<polyhydra::VCylinder>& connections,
                                  polyhydra::VMesh& mesh,
                                  OpenVolumeMesh::FaceHandle face,
                                  float scale,
                                  glm::vec4 color = glm::vec4(0.2, 0.2, 0.9, 1.0))
{
    auto& ovm = *mesh.get_ovm();
    auto control_points_prop = ovm.request_face_property<std::vector<double>>("BezierFaceControlPoints");
    int m = ovm.request_mesh_property<int>("BezierDegree")[OpenVolumeMesh::MH(0)];
    ...
    for (int i0 = 0; i0 <= m; i0++)
    {
        for (int i1 = 0; i1 <= m - i0; i1++)
        {
            ...
            for (auto& d_i : delta_i)
            {
                ...
                int to_i1 = i1 + (d_i1);
                int to_i2 = i2 + (d_i2);
                if (to_i0 >= 0 && to_i1 >= 0 && to_i2 >= 0 && to_i0 <= m && to_i1 <= m && to_i2 <= m)
                {
            ...
            {
                polyhydra::VCylinder connection = mesh.add_shape<polyhydra::VCylinder>();
...
void add_cp_connections_to_face(Face_Cylinder_Map& faces_connections,
                                polyhydra::VMesh& mesh,
                                OpenVolumeMesh::FaceHandle face,
                                float scale,
                                glm::vec4 color = glm::vec4(0.2, 0.2, 0.9, 1.0))
{
    std::vector<polyhydra::VCylinder> connections;
...
int main()
{
    set_theme(polyhydra::Theme::Dark);
    ...
    polyhydra::on_face_select(
        [&](polyhydra::VMesh mesh, OpenVolumeMesh::FaceHandle face)
        {
            if (!show_control_net_on_selection)
                return;
            ...
            if (is_bezier_mesh)
            {
                ...
                if (face_is_selected_it == is_selected.end())
                {
                    ...
                    add_cp_connections_to_face(meshes_selected_faces_connections.at(mesh_id),
                                               mesh,
                                               face,
                                               cp_connection_scale,
                                               connection_color);
                ...
                else
                {
                    ...
                    {
                        polyhydra::remove_shape(connection);
                    }
    ...
    polyhydra::on_gui_render(
        ...
        {
            ImGui::Begin("MyPanel");
            if (ImGui::Button("polyhydra:: Mesh"))
            {
                auto temp = polyhydra::load_from_dialog("Select OVM file");
            }
    ...
    std::string path = polyhydra::Internal::FileManager::get_resource_path().string()
                       + "/sample_meshes/bezier_meshes/bezier_spheroid_degree.4.ovm";
